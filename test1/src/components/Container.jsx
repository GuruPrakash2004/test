...
function Container({setValue,...props}){
    ...
    <div className="card course ms-2 bg-secondary justify-content-center">
            <img className="card-img-top p-1 rounded" src={props.image} alt="couesr1" />      
        <div className="couser-items card-body">
            <h3 className="card-title">{props.title}</h3>
            <p className="card-text">{props.desp}</p>
            <p><b>{props.rank}</b></p>
            <button onClick={()=>onToggle()} className="btn btn-outline-danger">click</button>
            <p><b>{istog ? "selected ": "not yet selected"}</b></p>
            <br />
            <h2>{count}</h2>
            <button onClick={Upgrade} className="btn btn-warning">up</button>
        </div>
    </div>
    </>
    )
